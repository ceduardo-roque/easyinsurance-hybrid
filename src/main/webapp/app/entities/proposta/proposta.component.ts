...
export default class Proposta extends mixins(JhiDataUtils) {
  ...
  public retrieveAllPropostas(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.propostas.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IProposta): void {
  ...
  public removeProposta(): void {
    this.propostaService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A Proposta is deleted with identifier ' + this.removeId;
